...
mutate(Survived=if_else(Survived==1,true="survived",false="non-survived"))%>%
...
#titanic$Survived<-as.factor(titanic$Survived)
#titanic$Age<-as.factor(as.numeric(titanic$Age))

survived_fit <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(Survived ~ Age, data = train_data, family = "binomial")

ggplot(data=titanic,mapping=aes(x=as.numeric(Age),y=Survived))+ geom_jitter(width = 0, height = 0.3, alpha = 0.5)

predict(survived_fit,test_data)

survived_pred <- predict(survived_fit, test_data, type = "prob") %>%
  bind_cols(test_data %>% select(Survived, Age))
survived_pred %>%
arrange(desc(.pred_survived)) %>%
print(n = 10)
```
...
titanic_m <- titanic %>%
  ...
  mutate(Survived=if_else(Survived == "survived",true = 1,false = 0)) %>%
  mutate(Survived=factor(Survived))

```
```{r remove-age-na, include=FALSE}
titanic_age_na <- titanic_m %>%
  filter(!is.na(Age))
```
```{r titanic-splitting, include=FALSE}
set.seed(1116)
titanic_split_m <- initial_split(titanic_age_na, prop = 0.80)
train_data_m <- training(titanic_split_m)
test_data_m  <- testing(titanic_split_m)
```
``` {r titanic-recipe, include=FALSE}
titanic_rec_m <- recipe(Survived ~ ., data = titanic_m) %>%
...
titanic_mod_m <- logistic_reg() %>% 
  set_engine("glm")
```
```{r titanic-workflow, include=FALSE}
titanic_wflow_m <- workflow() %>% 
add_model(titanic_mod_m) %>% 
add_recipe(titanic_rec_m)
```
### Modelling Survival
...
--
```{r titanic-fit-to-train, echo=FALSE}
titanic_fit_m <- titanic_wflow_m %>% 
  fit(data = train_data_m)
tidy(titanic_fit_m) %>%
kbl() %>%
...
.pull-right[
```{r ref.label="titanic-fit-to-train", echo=FALSE}
...
titanic_pred_m <-predict(titanic_fit_m, test_data_m, type = "prob") %>%
  bind_cols(test_data_m %>% select(Survived, PassengerId))
```
...
titanic_pred_m %>%
  mutate(
    ...
    titanic_pred_m  = if_else(.pred_1 > cutoff_prob_m, "Predicted to survive", "predicted not to survive")
    ) %>%
  count(titanic_pred_m, Survived) %>%
...
rsq(titanic_pred_m, truth = as.numeric(Survived), estimate = .pred_1)
rmse(titanic_pred_m, truth = as.numeric(Survived), estimate = .pred_1)
```
]
.pull-right[
+ add text here
